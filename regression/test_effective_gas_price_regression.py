...
def test_effective_gas_price(namespace, key, value):
    ...
    Asserts:
        transaction_response["gasPrice"] equals receipt_response["effectiveGasPrice"].
    """
    if key == "latest":
        value = get_indexed_latest_transaction(endpoints["blockscout"])

    transaction = base_request(
        namespace + "_getTransactionByHash",
        params=[value],
        endpoint=endpoints["shard_0"],
    )
    transaction_response = check_and_unpack_rpc_response(
        transaction, expect_error=False
    )
    ...
    assert transaction_response["gasPrice"] == receipt_response["effectiveGasPrice"]
